...
def mark_monthly_member_paid(group_id: str, player_id: str, principal: SupabasePrincipal = Depends(get_current_supabase_principal), db: Session = Depends(get_db_session)):
    return service.mark_monthly_member_paid(db, principal, group_id, player_id)


@router.delete('/monthly-members/{player_id}/mark-paid')
def unmark_monthly_member_paid(group_id: str, player_id: str, principal: SupabasePrincipal = Depends(get_current_supabase_principal), db: Session = Depends(get_db_session)):
    return service.unmark_monthly_member_paid(db, principal, group_id, player_id)
