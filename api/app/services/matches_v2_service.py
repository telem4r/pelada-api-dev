...
class MatchesV2Service:
    ...
    def _distribute_teams(self, pool: list[dict], *, players_per_team: int, team_count: int | None = None) -> dict[int, list[dict]]:
        ...
        # - mantém-se a exigência operacional mínima de 2 times completos para iniciar o sorteio.
        ...
        def full_size_teams(items: list[int]) -> list[int]:
            return [team for team in items if target_sizes[team] == players_per_team]
        ...
        def assign_item(item: dict, *, prioritize_goalkeeper: bool = False) -> None:
            ...
            if prioritize_goalkeeper:
                without_goalkeeper = [team for team in available if team_goalkeepers[team] == 0]
                if without_goalkeeper:
                    complete_without_goalkeeper = full_size_teams(without_goalkeeper)
                    available = complete_without_goalkeeper or without_goalkeeper
                best_team = min(
                    available,
                    key=lambda team: (
                        team_goalkeepers[team],
                        0 if target_sizes[team] == players_per_team else 1,
                        len(teams[team]),
                ...
                complete_available = full_size_teams(available)
