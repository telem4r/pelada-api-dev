...
def accept_invite(invite_id: str,
    ...
    # Guarda de idempotência. Nota: o enum membership_status_enum contém
    # apenas ('pending','active','rejected','removed') — convites aceitos
    # ficam com status = 'active' (não existe 'accepted' no schema).
    if invite['status'] in ('active', 'rejected', 'removed'):
