...
def _execute_draw_teams(
    *,
    ...
    def _assign_item(item: dict, *, prioritize_goalkeeper: bool = False) -> None:
        candidates = [idx for idx in range(team_count) if len(teams[idx]) < players_per_team]
        if not candidates:
            candidates = list(range(team_count))
        if prioritize_goalkeeper:
            preferred = [idx for idx in candidates if team_goalkeepers[idx] == 0]
            if preferred:
                candidates = preferred
        best_idx = min(candidates, key=lambda idx: (team_goalkeepers[idx] if prioritize_goalkeeper else 0, team_skill_totals[idx], len(teams[idx]), idx))
