...
Revision ID: 0013_safe_ensure_groups_columns
Revises: 0012_merge_final_heads
...
revision = "0013_safe_ensure_groups_columns"
down_revision = "0012_safe_ensure_groups_columns"
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    if not _table_exists(conn, "groups"):
        # Se por algum motivo a tabela ainda não existir, não tentamos “adivinhar”
        # a criação inteira aqui. Em deploy correto ela já existe via migrations anteriores.
        return

    # ---- campos criação do grupo ----
    ...
    # ---- multas ----
    _add_column_if_missing(conn, "groups", sa.Column("fine_enabled", sa.Boolean(), nullable=False, server_default=sa.text("false")))
...
def downgrade():
    # Não fazemos downgrade automático aqui porque é uma migration “safe ensure”.
    # Se precisar, removemos manualmente em uma migration específica.
    pass
