...
class MatchesV2Repository:
    ...
    def list_group_matches(self, db: Session, *, group_id: str, current_user_id: str | None = None) -> list[dict[str, Any]]:
        rows = db.execute(
            text(
                """
                with group_matches as (
                    select id from public.matches_v2
                ...
                presence as (
                    select
                        ...
                        bool_or(
                            ...
                            and mp.user_id = cast(:current_user_id as uuid)
                            and mp.status = 'confirmado'
                        ) as current_user_confirmed
                    from public.match_participants_v2 mp
                    where mp.match_id in (select id from group_matches)
                ...
                guest_presence as (
                    ...
                    where mg.match_id in (select id from group_matches)
