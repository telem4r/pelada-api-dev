...
class MatchesV2Service:
    ...
    def _distribute_teams(self, pool: list[dict], *, players_per_team: int, team_count: int | None = None) -> dict[int, list[dict]]:
        ...
        # Regra de negócio do BoraFut:
        # - o sorteio usa todos os participantes confirmados cuja chegada foi marcada pelo Owner/Admin;
        # - pagamento não é critério de sorteio;
        # - ninguém elegível deve ser descartado;
        # - players_per_team é a capacidade total definida pelo Owner/Admin, incluindo goleiro;
        # - times completos são montados primeiro e a sobra forma o último time;
        # - o equilíbrio deve considerar skill proporcional ao tamanho de cada time.
        ...
        # A quantidade de times deriva da quantidade de elegíveis e da capacidade definida pelo Owner/Admin.
        # O campo legacy team_count não deve forçar times vazios nem excluir jogadores.
        resolved_team_count = max(2, math.ceil(len(eligible) / players_per_team))

        target_sizes: dict[int, int] = {}
        remaining = len(eligible)
        for team_number in range(1, resolved_team_count + 1):
            target_size = min(players_per_team, max(remaining, 0))
            target_sizes[team_number] = target_size
            remaining -= target_size

        teams: dict[int, list[dict]] = {team_number: [] for team_number in range(1, resolved_team_count + 1)}
        ...
        def normalize_position(item: dict) -> str:
            ...
            if raw in {'goalkeeper', 'keeper', 'gk', 'goal', 'goleiro', 'gol'}:
                return 'goleiro'
            return 'linha'
        ...
        total_skill = sum(skill_value(item) for item in eligible)
        total_players = max(1, len(eligible))
        ideal_skill_totals = {
            team: (total_skill * target_sizes[team] / total_players) if target_sizes[team] > 0 else 0.0
            for team in teams
        }

        def teams_with_capacity() -> list[int]:
            return [team for team, players in teams.items() if len(players) < target_sizes[team]]

        def objective() -> float:
            # Balanceia skill proporcional ao tamanho do time. Um time de sobra com 3 jogadores
            # não deve ter a mesma soma de skill de um time completo, mas deve ter média comparável.
            return sum(abs(team_skill_totals[team] - ideal_skill_totals[team]) for team in teams)

        def add_to_team(team: int, item: dict) -> None:
            teams[team].append(item)
            team_skill_totals[team] += skill_value(item)
            if normalize_position(item) == 'goleiro':
                team_goalkeepers[team] += 1

        def select_team_for_goalkeeper(item: dict, preferred_teams: list[int] | None = None) -> int:
            available = [team for team in (preferred_teams or teams_with_capacity()) if len(teams[team]) < target_sizes[team]]
            if not available:
                raise HTTPException(status_code=500, detail='Falha interna ao distribuir goleiros no sorteio.')
            item_skill = skill_value(item)
            return min(
                available,
                key=lambda team: (
                    team_goalkeepers[team],
                    abs((team_skill_totals[team] + item_skill) - ideal_skill_totals[team]),
        ...
        def select_team_for_line_player(item: dict) -> int:
            available = teams_with_capacity()
            if not available:
                raise HTTPException(status_code=500, detail='Falha interna ao distribuir jogadores no sorteio.')
            item_skill = skill_value(item)
            return min(
                available,
                key=lambda team: (
                    abs((team_skill_totals[team] + item_skill) - ideal_skill_totals[team]),
                    (len(teams[team]) + 1) / max(1, target_sizes[team]),
                    team_skill_totals[team],
                    team,
                ),
            )

        goalkeepers = ordered_items([item for item in eligible if normalize_position(item) == 'goleiro'])
        line_players = ordered_items([item for item in eligible if normalize_position(item) != 'goleiro'])

        # Primeira passagem: tenta colocar até 1 goleiro por time, priorizando times completos.
        # Se houver menos goleiros que times, a sobra fica sem goleiro; se houver mais, os extras
        # são distribuídos depois por equilíbrio de skill/capacidade.
        primary_goalkeeper_teams = sorted(
            teams.keys(),
            key=lambda team: (-target_sizes[team], ideal_skill_totals[team], team),
        )
        for item, preferred_team in zip(goalkeepers[:len(primary_goalkeeper_teams)], primary_goalkeeper_teams):
            add_to_team(preferred_team, item)

        for item in goalkeepers[len(primary_goalkeeper_teams):]:
            add_to_team(select_team_for_goalkeeper(item), item)

        for item in line_players:
            add_to_team(select_team_for_line_player(item), item)

        # Hardening final: procura a melhor troca possível entre jogadores de linha,
        # aplica uma troca por ciclo e reinicia a varredura. Isto evita mutação da lista
        # durante iteração profunda e preserva tamanhos/goleiros.
        while True:
            current_objective = objective()
            best_swap: tuple[int, int, int, int, float] | None = None

            for team_a in sorted(teams):
                for team_b in sorted(teams):
                    if team_a >= team_b:
                        continue
                    for idx_a, item_a in enumerate(teams[team_a]):
                        if normalize_position(item_a) == 'goleiro':
                            continue
                        for idx_b, item_b in enumerate(teams[team_b]):
                            if normalize_position(item_b) == 'goleiro':
                                continue
                            skill_a = skill_value(item_a)
                            skill_b = skill_value(item_b)
                            if skill_a == skill_b:
                                continue

                            projected_a = team_skill_totals[team_a] - skill_a + skill_b
                            projected_b = team_skill_totals[team_b] - skill_b + skill_a
                            new_objective = current_objective
                            new_objective -= abs(team_skill_totals[team_a] - ideal_skill_totals[team_a])
                            new_objective -= abs(team_skill_totals[team_b] - ideal_skill_totals[team_b])
                            new_objective += abs(projected_a - ideal_skill_totals[team_a])
                            new_objective += abs(projected_b - ideal_skill_totals[team_b])

                            if new_objective + 0.0001 < current_objective:
                                if best_swap is None or new_objective < best_swap[4]:
                                    best_swap = (team_a, idx_a, team_b, idx_b, new_objective)

            if best_swap is None:
                break

            team_a, idx_a, team_b, idx_b, _ = best_swap
            item_a = teams[team_a][idx_a]
            item_b = teams[team_b][idx_b]
            skill_a = skill_value(item_a)
            skill_b = skill_value(item_b)
            teams[team_a][idx_a], teams[team_b][idx_b] = item_b, item_a
            team_skill_totals[team_a] += skill_b - skill_a
            team_skill_totals[team_b] += skill_a - skill_b
