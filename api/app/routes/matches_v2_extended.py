...
def _ensure_open_match_obligation_for_target(db: Session, *, group_id: str, match_id: str, target: str, target_id: str, actor_user_id: str) -> dict[str, Any] | None:
    ...
    if not finance or finance.get('group_type') != 'avulso':
        return None
...
def close_match(group_id: str, match_id: str,
    ...
    if generate_charges:
        ...
        group = db.execute(text("""
            select coalesce(single_cost, 0) as single_cost, coalesce(venue_cost, 0) as venue_cost, currency
            from public.groups where id = cast(:gid as uuid)
        """), {'gid': group_id}).mappings().first()

        if group and float(group['single_cost'] or 0) > 0:
            # Generate per-player obligations for confirmed participants
            participants = db.execute(text("""
                ...
                where p.match_id = cast(:mid as uuid) and p.status = 'confirmado'
            """), {'mid': match_id}).mappings().all()

            for part in participants:
                db.execute(text("""
                    INSERT INTO public.finance_obligations_v2
                        (id, group_id, user_id, player_id, match_id, source_type, title, amount, currency, status, created_at)
                    VALUES (gen_random_uuid(), cast(:gid as uuid),
                            case when :uid is not null then cast(:uid as uuid) end,
                            case when :pid is not null then cast(:pid as uuid) end,
                            cast(:mid as uuid), 'match',
                            'Partida - cobrança avulso', :amount, :currency, 'pending', now())
                    ON CONFLICT DO NOTHING
                """), {
                    'gid': group_id, 'uid': part.get('user_id'), 'pid': part.get('player_id'),
                    'mid': match_id, 'amount': float(group['single_cost']),
                    'currency': group['currency'] or 'BRL',
                })
...
def admin_mark(group_id: str, match_id: str,
    ...
    finance = _group_finance_context(db, group_id) or {}
    is_avulso_group = (finance.get('group_type') or '').strip().lower() == 'avulso'
    ...
    if target == 'guest':
        ...
        if updates:
            db.execute(text(f"""
                ...
                WHERE id = cast(:gid as uuid) AND match_id = cast(:mid as uuid)
            """), params)
        if arrived is True and is_avulso_group:
